...
export const ThreeScene: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const createShape = (formFn: (i: number) => { x: number; y: number; z: number }) => {
    ...
    const spherePositions = createShape((i) => {
      const u = Math.random();
      const v = Math.random();
    ...
    const singularityPositions = createShape((i) => {
      const angle = Math.random() * Math.PI * 2;
    ...
    const animate = () => {
      ...
      const dustArray = dustGeometry.getAttribute('position').array as Float32Array;
      for (let i = 0; i < 500; i++) {
      ...
      for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        const ripple = Math.sin(elapsedTime * 2 + basePosValX * 2 + basePosValY * 2) * 0.04;
